...
```{r}
rm(list=ls())
savepath = "D:/Analysis/EMT/ceres_robpca.RData"
load(savepath)
```
...
Now let's recast the dataframe.
```{r}
#global_pvals = matrix(global_pvals, 
#                      nrow=dim(ceres)[1], 
#                      ncol=dim(ceres)[2],
#                      byrow=TRUE)
#global_pvals = data.frame(global_pvals)
#global_pvals$Var1 = mlt_ceres$Var1
#global_pvals$Var2 = mlt_ceres$Var2

#tmp = dcast(global_pvals, 
#                        formula=Var1~Var2, 
#                        value.var=global_pvals)
