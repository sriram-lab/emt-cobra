...
```{r, include=TRUE}
plot_histograms = function(data, labels, slot){
  plts = list()
  exp_data = list()
...
#### Plot of the scRNASeq scaled data
Create data objects containing histograms.
```{r, include=TRUE}
...
```{r, include=TRUE}
features = c("SAMD11", "PLEKHN1")
RidgePlot(object=ovca420_tgfb, features=features, ncol=2)
VlnPlot(object=ovca420_tgfb, features=features)
DotPlot(object=ovca420_tgfb, genes=features, legend=TRUE)
FeaturePlot(object=ovca420_tgfb, features = features)
#DoHeatmap(object = SubsetData(object=ovca420_tgfb, max.cells.per.ident = 100), genes.use= features, slim.col.label=TRUE, group.label.rot=TRUE)
```
