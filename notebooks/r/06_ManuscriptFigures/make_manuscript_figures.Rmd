...
```{r}
filename = "D:/Chandrasekaran/Projects/EMT/Analysis/COBRA/Manuscript/Bulk Flux Intersection.csv"
...
```{r}
# Split by hours
early_data = as.matrix(abs(top50[, c("GSE17708_1hrs", "Garcia_1hrs")]))
mid1_data  = as.matrix(abs(top50[, c("GSE17708_8hrs")]))
mid2_data  = as.matrix(abs(top50[, c("GSE17708_24hrs", "Garcia_24hrs")]))
late_data  = as.matrix(abs(top50[, c("GSE17708_72hrs", "GSE17518_72hrs", "Garcia_48hrs", 
                       "Keshamouni_72hrs")]))
```
...
hm = Heatmap(as.matrix(expt_data),
             col = colorRamp2(c(1, 2, 3, 4, 5), 
                              c("#D8F3DC", "#95D5B2", "#52B788", "#2D6A4F", "#1B4332")),
...
```{r, fig.width=20, fig.height=12.5, fig.align='right', dpi=100}
png(filename="D:/Chandrasekaran/Projects/EMT/Analysis/Figures/COBRA Predictions/RECON1_flux_heatmap.png",
       width=20,
       height=12.5,
       units='in',
    res=1200)
...
flux_heatmap = function(df, pathway){
  ...
  mid1_data  = abs(data[, c("GSE17708_8hrs")])
  mid2_data  = abs(data[, c("GSE17708_24hrs", "Garcia_24hrs")])
...
png(filename="D:/Chandrasekaran/Projects/EMT/Analysis/Figures/COBRA Predictions/glycolysis_flux_heatmap.png",
       width=20,
       height=12.5,
       units='in',
    res=1200)
...
I need to make a custom script to specifically bold and red color the enolase reaction for the glycolysis map.
...
# Split by hours
early_data = abs(tmp2[, c("GSE17708_1hrs", "Garcia_1hrs")])
mid1_data  = abs(tmp2[, c("GSE17708_8hrs")])
mid2_data  = abs(tmp2[, c("GSE17708_24hrs", "Garcia_24hrs")])
late_data  = abs(tmp2[, c("GSE17708_72hrs", "GSE17518_72hrs", "Garcia_48hrs", 
                                   "Keshamouni_72hrs")])
tmp = cbind(early_data, mid1_data, mid2_data, late_data)
...
# Rows to highlight
myRows = c('enolase, Glycolysis/Gluconeogenesis')

# Set stylings for row names and make our selected rows unique
row_idx = which(tmp2$name %in% myRows)
fontcolors = rep('black', nrow(tmp2))
fontcolors[row_idx] = 'red'
fontfaces = rep('plain',nrow(tmp2))
fontfaces[row_idx] = 'bold'
rowAnno = rowAnnotation(rows = anno_text(tmp2$name, 
                                         gp = gpar(fontface = fontfaces, 
                                                   col = fontcolors),
                                         just='right', 
                                         location=1))


h1 = Heatmap(row_names_side="left",
             early_data, 
             col=col_fun, 
             cluster_columns=FALSE, 
             rect_gp = gpar(col="black", 
...
### E. Visualize TCA Cycle Reactions
Now let's specifically get reactions related to Glycolysis.
```{r, fig.width=20, fig.height=12.5, fig.align='right', dpi=100}
tca_figures = flux_heatmap(data, pathway="Citric Acid Cycle")
png(filename="D:/Chandrasekaran/Projects/EMT/Analysis/Figures/COBRA Predictions/tca_flux_heatmap.png",
       width=20,
       height=12.5,
       units='in',
    res=1200)
...
### F. Visualize OXPHOS Reactions
Now let's specifically get reactions related to Glycolysis.
```{r, fig.width=20, fig.height=12.5, fig.align='right', dpi=100}
oxphos_figures = flux_heatmap(data, pathway="Oxidative Phosphorylation")
png(filename="D:/Chandrasekaran/Projects/EMT/Analysis/Figures/COBRA Predictions/oxphos_flux_heatmap.png",
       width=20,
       height=12.5,
       units='in',
    res=1200)
...
### F. Visualize Glutamate Reactions
Now let's specifically get reactions related to Glycolysis.
```{r, fig.width=20, fig.height=12.5, fig.align='right', dpi=100}
gln_figures = flux_heatmap(data, pathway="Glutamate metabolism")
png(filename="D:/Chandrasekaran/Projects/EMT/Analysis/Figures/COBRA Predictions/gln_flux_heatmap.png",
       width=20,
       height=12.5,
       units='in',
    res=1200)
...
dev.off()
```
### G. Visualize Single Cell Reactions Reactions
Now let's specifically get reactions related to single-cell time points.
```{r, fig.width=20, fig.height=12.5, fig.align='right', dpi=100}
# If pathway variable is missing, just visualize everything
tmp2 = data
...
# Split by hours
mid1_data  = abs(tmp2[, c("GSE147405_8hrs")])
mid2_data  = abs(tmp2[, c("GSE147405_24hrs")])
late_data  = abs(tmp2[, c("GSE147405_72hrs")])
tmp = cbind(mid1_data, mid2_data, late_data)
...
h2 = Heatmap(row_names_side="left",t
             mid1_data, 
             col=col_fun, 
             cluster_columns=FALSE, 
             rect_gp = gpar(col="black", 
...
### A. Load Data
```{r}
filename = "D:/Chandrasekaran/Projects/EMT/Analysis/COBRA/Manuscript/Bulk Reaction KO Intersection.csv"
data = read_csv(filename)
...
ko_heatmap = function(df, pathway){
  ...
  mid1_data  = data[, c("GSE17708_8hrs")] - 1
  mid2_data  = data[, c("GSE17708_24hrs", "Garcia_24hrs")] - 1
...
png(filename="D:/Chandrasekaran/Projects/EMT/Analysis/Figures/COBRA Predictions/RECON1_rxnko_heatmap.png",
       width=20,
       height=12.5,
       units='in',
    res=1200)
...
# Rows to highlight
myRows = c('L-Glutamine exchange, NA',
           'D-Glucose exchange, NA',
           'pyruvate carboxylase, Pyruvate Metabolism',
           'C160 fatty acid activation, Fatty acid activation',
           'hexokinase (D-glucose:ATP), Glycolysis/Gluconeogenesis',
           'aspartate transaminase, Alanine and Aspartate Metabolism')

# Set stylings for row names and make our selected rows unique
row_idx = which(data$name %in% myRows)
fontcolors = rep('black', nrow(data))
fontcolors[row_idx] = 'red'
fontfaces = rep('plain',nrow(data))
fontfaces[row_idx] = 'bold'
rowAnno = rowAnnotation(rows = anno_text(data$name, 
                                       gp = gpar(fontface = fontfaces, 
                                                 col = fontcolors),
                                       just='right', 
                                       location=1))
...
# Create figures
specific_figures = ko_heatmap(specific_data)
png(filename="D:/Chandrasekaran/Projects/EMT/Analysis/Figures/COBRA Predictions/RECON1_rxnko_specific_heatmap.png",
       width=20,
       height=12.5,
       units='in',
    res=1200)
...
To make a custom figure for the manuscript:
```{r, fig.width=20, fig.height=12.5, fig.align='right', dpi=100}

tmp2 = specific_data
# Rows to delete:
rows_to_delete = c(33, 38)
tmp2 = tmp2[-rows_to_delete, ]
...
# Split by hours
early_data = tmp2[, c("GSE17708_1hrs", "Garcia_1hrs")] - 1
mid1_data  = tmp2[, c("GSE17708_8hrs")] - 1
mid2_data  = tmp2[, c("GSE17708_24hrs", "Garcia_24hrs")] - 1
late_data  = tmp2[, c("GSE17708_72hrs", "GSE17518_72hrs", "Garcia_48hrs", 
                                   "Keshamouni_72hrs")] - 1
tmp = cbind(early_data, mid1_data, mid2_data, late_data)
...
# Rows to highlight
myRows = c('enolase, Glycolysis/Gluconeogenesis',
           'ATP-Citrate lyase, Citric Acid Cycle',
           'carnitine O-aceyltransferase, mitochondrial, Fatty Acid Metabolism',
           'fatty acyl-CoA synthase (n-C10:0CoA), Fatty acid elongation',
           'fatty-acyl-CoA synthase (n-C12:0CoA), Fatty acid elongation',
           'fatty-acyl-CoA synthase (n-C14:0CoA), Fatty acid elongation',
           'pantetheine-phosphate adenylyltransferase, CoA Biosynthesis',
           'carnitine O-acetyltransferase, Carnitine shuttle',
           'alpha-ketoglutarate/malate transporter, Transport, Mitochondrial',
           'Aconitate hydratase, Citric Acid Cycle',
           'Isocitrate dehydrogenase (NADP+), Citric Acid Cycle',
           'L-lactate dehydrogenase, Glycolysis/Gluconeogenesis',
           'malate dehydrogenase, Citric Acid Cycle'
           )
...
# Construct the heatmaps
col_fun = colorRamp2(c(min(as.matrix(tmp)), -0.005, 0, 0.03), 
                     c("#E63946", "#F1959B", "Gray", "#3E3B39")) 
h1 = Heatmap(row_names_side="left",
             early_data, 
             col=col_fun, 
             cluster_columns=FALSE, 
...
### A. Load Data
```{r}
filename = "D:/Chandrasekaran/Projects/EMT/Analysis/data/scRNASeq Reaction KO.csv"
data = read_csv(filename)
...
hm = Heatmap(as.matrix(data[, "S.D."]),
             ...
             width = ncol(data[, "S.D."])*unit(5, "mm"), 
             height = nrow(data[, "S.D."])*unit(5, "mm"))
```

### D. Create the Heatmap with additional figures
```{r, fig.width=20, fig.height=12.5, fig.align='right', dpi=100}
final_data = as.matrix(tmp)
col_fun = colorRamp2(c(min(as.matrix(tmp)), -0.05, 0, 0.03), 
                     c("#E63946", "#F1959B", "Gray", "#3E3B39")) 
...
ht = Heatmap(row_labels=data$name,
             ...
             col = col_fun,
             heatmap_legend_param=list(title="Normalized KO Growth Score",
                                       at = c(min(as.matrix(tmp)), 0, round(max(as.matrix(tmp)), 4))),
             width = ncol(final_data)*unit(5, "mm"), 
             height = nrow(final_data)*unit(5, "mm")) + hm 

ht
```
Save figures
```{r}
png(filename="D:/Chandrasekaran/Projects/EMT/Analysis/Figures/COBRA Predictions/scRNASeq_avg_rxn_ko.png",
       width=20,
       height=12.5,
       units='in',
    res=1200)
...
## 4. Glycolysis Expression Matrix
