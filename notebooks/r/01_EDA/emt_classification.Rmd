...
Subset data based on quantiles
```{r}
cmb$State = "I"
cmb$State[cmb$numSig <=5] = "E"
cmb$State[cmb$numSig >= 8] = "M"

cmb$state_encoding = NA
cmb$state_encoding[cmb$State == "E"] = 0
cmb$state_encoding[cmb$State == "I"] = 1
cmb$state_encoding[cmb$State == "M"] = 1

tmp = cmb[!is.na(cmb$state_encoding), ] 
tmp = tmp[tmp$State != "I", ] 
```
...
tmp = merge(tmp, col2get, by.x=0, by.y="DepMap_ID")
...
library(yardstick)
tmp$pm_encoding = 0
tmp$pm_encoding[tmp$primary_or_metastasis == "Metastasis"] = 1

tmp$state_encoding = as.factor(tmp$state_encoding)
tmp$pm_encoding = as.factor(tmp$pm_encoding)
...
```{r}
cm = conf_mat(data = tmp, estimate=state_encoding, truth=pm_encoding)
cm
```
...
Save the data
```{r}
write.csv(tmp, "filtered_EMT_Marker_annot.csv")
```
