...
Let's create the annotation bars.
```{r}
...
```{r, fig.width=12.5, fig.height=40, fig.align='right', dpi=100}
# Metastatic heatmap
col_fun = colorRamp2(c(max(masked_met_data, na.rm=TRUE), median(masked_met_data, na.rm=TRUE), min(masked_met_data, na.rm=TRUE)), 
...
hm5 = Heatmap(as.matrix(tmp1),
              ...
              cluster_rows=FALSE,
              bottom_annotation=h1_col, 
              width = ncol(tmp1)*unit(5, "mm"), 
              height = nrow(tmp1)*unit(5, "mm"),
              show_heatmap_legend=FALSE)
...
hm6 = Heatmap(as.matrix(tmp2),
              ...
              cluster_rows=FALSE,
              bottom_annotation=h2_col, 
              width = ncol(tmp2)*unit(5, "mm"), 
              height = nrow(tmp2)*unit(5, "mm"),
              heatmap_legend_param=list(title="CERES Score"))
...
This section visualizes the metabolic genes that are predicted to be sensitive from gene or reaction knockout, and that have high metabolic activity.
