function [fluxstate_gurobi,grate, solverobj, inconsistencies, model] =  constrain_flux_regulation_inconsistencies(model1,onreactions,offreactions,kappa,rho,epsilon,epsilon2,mode,minfluxflag)
...
try
    ...
    if length(onreactions) > 0
        ...
        inconsistencies.on = sum(fluxstate_gurobi(rxnpos) < epsilon); % these are supposed to be above epsilon but they are below
    else
    ...
    if length(offreactions_init) > 0
        ...
        inconsistencies.off = sum(fluxstate_gurobi(rxnpos) > epsilon2_init); % these are supposed to be below epsilon2 but are higher
    else
